...
def get_users():
    return {"1": {"login": "user", "password": "qwerty"}}
...
def load_user(user_id):
    ...
    if user_data:
        return User(user_id, user_data['login'])
    return None
...
def login():
    if request.method == "POST":
        ...
        users = get_users()
        user = next((user_data for user_data in users.values() if user_data['login'] == login and user_data['password'] == password), None)
        if user:
            login_user(User("1", user['login']), remember=remember)
